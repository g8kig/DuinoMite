    ...
    void cmd_name(void) {
        ...
        { // start a new block
            ...
            if (GetDrive(old) == FLASHFS) {
                ...
                if (strlen(old) == 0 || strlen(old) > 12 || strlen(new) == 0 || strlen(new) > 12) {
                    ErrorThrow(5);
                    return;
                }
                if (!InitSDCard()) return;
